...
public readonly struct Commands
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Send<T>(T triggerStruct = default) where T : class, ITrigger
    {
        World.Send(triggerStruct);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Receive<T>(Action<T> action) where T : class, ITrigger
    {
    ...
    public bool TryGetElement<T>(out T element) where T : class, IElement
    {
